...
%>
	...
	function ClearAll(){
		...
		FCKeditorAPI.GetInstance('FCKeditor1').PutHTML(' ');
	}
		...
		if (isSucceed) {
			if (op=="edit")
			{
				...
				if (confirm("<%=correct_result%> 点击确定将转至文章首页，点击取消继续添加！")) {
					window.location.href = "fckwebedit.jsp?op=edit&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&id=<%=id%>";
				}
				else
					window.location.reload(true);
		    }
...
<%if (templateId!=-1 && op.equals("add")) {%>	
	FCKeditorAPI.GetInstance('FCKeditor1').value = divTemplate.innerHTML;
<%}else if (op.equals("edit")){%>
	FCKeditorAPI.GetInstance('FCKeditor1').value = addform.content.value;
<%}%>
}
//-->
</script>
          ...
          <TABLE cellSpacing=0 cellPadding=0 width="100%">
            <TBODY>
              <TR>
				...
				<%
				if (op.equals("add")) {%>
					<lt:Label res="res.label.doc_editpage" key="add_content"/>&nbsp;--&nbsp;<a href="fckwebedit.jsp?op=edit&dir_code=<%=doc.getDirCode()%>&id=<%=doc.getID()%>"><%=doc.getTitle()%>（<lt:Label res="res.label.doc_editpage" key="return_front_page"/>）</a>
				    ...
				    <%}else{%>
					<lt:Label res="res.label.doc_editpage" key="modify_content"/>&nbsp;--&nbsp;<a href="fckwebedit.jsp?op=edit&dir_code=<%=doc.getDirCode()%>&id=<%=doc.getID()%>"><%=doc.getTitle()%></a>
				<%}%>
				</TD>
              </TR>
            </TBODY>
...
<%if (doc!=null && doc.getTemplateId()!=-1) {%>
			...
			<a href="javascript:oFCKeditor.Value = divTemplate.innerHTML"><lt:Label res="res.label.doc_editpage" key="renew_apply_template"/></a>
			</span>
			</td>
            </tr>
<%}%>
...
if (op.equals("edit")) {
%>
<%=strutil.getNullString(doc.getContent(pageNum))%>
<%}%>
</pre>
...
&nbsp;
      <%if (op.equals("edit")) {%>
	  <input name="editbtn" type="button" onClick="location.href='doc_fck_abstract.jsp?id=<%=doc.getID()%>'" value=" <%=SkinUtil.LoadString(request,"res.label.doc_editpage","abstract")%> ">
	  <%
...
&nbsp;<input name="button" type="button" onClick='window.open("doc_view_create.jsp?id=<%=id%>&isCreateHtml=true&CPages=<%=pageNum%>")' value="预览并生成"></td>
		...
		<table width="100%"  border="0">
          <tr>
            <td align="center">
			<a href="fckwebedit.jsp?op=editarticle&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&id=<%=id%>"><lt:Label res="res.label.doc_editpage" key="artical_info"/></a>
			　<lt:Label res="res.label.doc_editpage" key="artical_count"/><%=doc.getPageCount()%><lt:Label res="res.label.doc_editpage" key="pages"/>&nbsp;&nbsp;<lt:Label res="res.label.doc_editpage" key="page"/>
					...
					&nbsp;<a href="doc_fck_editpage.jsp?op=add&doc_id=<%=doc.getID()%>"><%=SkinUtil.LoadString(request,"res.label.doc_editpage","add_page")%></a>			
			</td>
          </tr>
        </table>
	<br></TD>
