...
if (curLeaf==null) {
	out.print(SkinUtil.makeErrMsg(request, "版块不存在！"));
	return;
}

if (!Privilege.isUserCanCheck(request, boardcode)) {
...
<html>
<title>审核贴子 - <%=Global.AppName%></title>
...
if (op.equals("check")) {
	...
	out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "topic_check_board.jsp?boardcode=" + StrUtil.UrlEncode(boardcode) + "&checkStatus=" + checkStatus));
	return;
}
...
<table width='100%' cellpadding='0' cellspacing='0' >
  <tr>
    <td class="head"><lt:Label res="res.label.forum.topic_m" key="check"/>&nbsp;-&nbsp;<%=curLeaf.getName()%></td>
  </tr>
</table>
<CENTER>
  <br>
...
style="PADDING-RIGHT: 0px; BORDER-TOP: 1px; PADDING-LEFT: 0px; PADDING-BOTTOM: 0px; BORDER-LEFT: 1px; PADDING-TOP: 0px; BORDER-BOTTOM: 1px; BORDER-COLLAPSE: collapse; BORDER-RIGHT-WIDTH: 1px">
    <FORM name=form1 action="topic_check_board.jsp" method=get onSubmit="return form1_onsubmit()">
      <TBODY>
        <TR>
          <TD height=22 colSpan=2 class="thead" >
          <lt:Label res="res.label.forum.search" key="input_keywards"/></TD>
        </TR>
        <TR bgColor=#f5f5f5>
    ...
    align=right width=210 bgColor=#f5f5f5 height=26><FONT 
      style="FONT-SIZE: 9pt" color=#000000>&nbsp;&nbsp;</FONT></TD>
  ...
  <%
		...
		if (searchtype.equals("byauthor")) {
			...
			if ( selauthor.equals("topicname"))
				...
				sql = "select id from sq_message where boardcode=" + StrUtil.sqlstr(boardcode) + " and id in (select rootid from sq_message where name in (" + nicks + "))";
			else
				sql = "select id from sq_message where boardcode=" + StrUtil.sqlstr(boardcode) + " and name in (" + nicks + ")";
		}
		else if (searchtype.equals("bykey")) {
			sql = "select id from sq_message where boardcode=" + StrUtil.sqlstr(boardcode) + " and title like " + StrUtil.sqlstr("%"+searchwhat+"%");
		}
		else
			sql = "select id from sq_message where boardcode=" + StrUtil.sqlstr(boardcode);
...
function doDel() {
	...
	window.location.href = "topic_check_board.jsp?op=del&boardcode=<%=StrUtil.UrlEncode(boardcode)%>&checkStatus=<%=checkStatus%>&ids=" + ids;
}
...
function doCheck() {
	...
	window.location.href = "topic_check_board.jsp?op=check&boardcode=<%=StrUtil.UrlEncode(boardcode)%>&checkStatus=<%=checkStatus%>&ids=" + ids;
}
