...
<%		
	...
	if (op.equals("del")) {
		...
		String sql = "delete from " + bvld.getTable().getName() + " where boardcode<>" + StrUtil.sqlstr(Leaf.CODE_BLOG) + " add_date<=?";
		if (beginDate.equals("")) {
	...
	String sql = "select count(*) as s, topic_id from " + bvld.getTable().getName() + " where boardcode<>" + StrUtil.sqlstr(Leaf.CODE_BLOG);
	JdbcTemplate jt = new JdbcTemplate();
...
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
  <tr> 
    <td height=20 align="left" class="thead">贴子访问统计</td>
  </tr>
  <tr> 
    <td valign="top"><br>
...
<input name="button" type="button" onClick="del()" value="删除"></TD>
      ...
      <tr align="center" bgcolor="#F8F7F9">
        <td width="50%" height="24" bgcolor="#EFEBDE">
          贴子        </td>
        <td width="20%" bgcolor="#EFEBDE">作者</td>
        <td width="15%" bgcolor="#EFEBDE"> 访问量 </td>
        <td width="15%" bgcolor="#EFEBDE">日访问量</td>
        </tr>
	<%
	...
	%>
      <tr align="center">
		...
		<a href="../showtopic.jsp?rootid=<%=md.getId()%>" target="_blank"><%=title%></a>		</td>
        ...
        <td bgcolor="#FFF7FF"><a href="visit_topic_statistic_day_detail.jsp?id=<%=md.getId()%>&beginDate=<%=StrUtil.UrlEncode(beginDate)%>&endDate=<%=StrUtil.UrlEncode(endDate)%>">查看</a></td>
        </tr>
	<%}%>
    </table>
  ...
  <tr>
    <td align="center"><%
JFreeChart chart = ChartFactory.createBarChart3D("贴子访问统计图", 
                  "贴子",
                  "访问量",
                  dataset,
