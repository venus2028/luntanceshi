...
var basePath = "<%=Global.getFullRootPath()%>/forum";
...
%>
<script>
window.parent.leftFileFrame.location.href="media_left.jsp";
</script>
<%	
	return;
}
...
if (op.equals("upload")) {
	...
	try {
		re = isfm.create(application, request);
		if (re)
			out.print(StrUtil.Alert_Redirect("操作成功！", "media_list.jsp?action=" + action + "&dirCode=" + StrUtil.UrlEncode(dirCode)));	
		else
			out.print(StrUtil.Alert_Redirect("操作失败！", "media_list.jsp?action=" + action + "&dirCode=" + StrUtil.UrlEncode(dirCode)));		
	}
	catch (ErrMsgException e) {
		out.print(StrUtil.Alert_Redirect(e.getMessage(), "media_list.jsp?action=" + action + "&dirCode=" + StrUtil.UrlEncode(dirCode)));	
		return;
	}
}
...
if (op.equals("rename")) {
	...
	try {
		...
		if (re)
			...
			out.print(StrUtil.Alert_Redirect("操作失败！", "media_list.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));		
	}
	catch (ErrMsgException e) {
		out.print(StrUtil.Alert_Redirect(e.getMessage(), "media_list.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));	
		return;
	}
}
	  ...
	  if (!op.equals("search")) {
	  	if (leaf!=null && leaf.isLoaded()) {
			...
			while (!parentcode.equals("root")) {
				...
				navstr = "<a href='media_list.jsp?action=" + action + "&dirCode=" + StrUtil.UrlEncode(plf.getCode()) + "'>" + plf.getName() + "</a>&nbsp;>>&nbsp;" + navstr;
		...
		%>
      &nbsp;&nbsp;<a href="media_list_pic.jsp?dirCode=<%=StrUtil.UrlEncode(dirCode)%>&action=<%=action%>">缩略图</a></td>
    </tr>
  </tbody>
</table>
<br>
<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0" class="p9">
  <form name="form1" action="media_list.jsp?op=search" method="post">
    <tr>
...
%>
    <tr>
      <td colspan="4" align="left" nowrap class="tbg1" style="PADDING-LEFT: 5px; height:22px"><img src="images/folder_01.gif" width="16" height="12">&nbsp;<a href="media_list.jsp?dirCode=<%=StrUtil.UrlEncode(leaf.getParentCode())%>">..</a></td>
    </tr>	
<%
}
...
%>	
    <tr>
      <td colspan="4" align="left" nowrap class="tbg1" style="PADDING-LEFT: 5px; height:22px"><img src="images/folder_01.gif" width="16" height="12">&nbsp;<a href="media_list.jsp?action=<%=action%>&dirCode=<%=StrUtil.UrlEncode(isdd.getCode())%>"><%=isdd.getName()%></a></td>
    </tr>	
<%}%>	
<%
...
while (ri.hasNext()) {
	...
	k++;
	%>
  <form name="form<%=k%>" action="media_list.jsp?op=rename" method="post">
    <tr onMouseOver="this.className='tbg1sel'" onMouseOut="this.className='tbg1'" class="tbg1" style="padding-left:5px">
	  ...
	  <a class="tooltips" href="#">
		...
		<span><img src="../<%=isfd.getVisualPath() + "/" + isfd.getDiskName()%>"></span>
		<%}else{%>
		<span>
	  ...
	  <a href="#" onClick="selectImage('<%=isfd.getVisualPath() + "/" + isfd.getDiskName()%>')">选择</a>&nbsp;
	  <%}%>
	  <a href="#" onClick="form<%=k%>.submit()">重命名</a>&nbsp;<a href="#" onClick="if (confirm('您确定要删除吗？')) window.location.href='media_list.jsp?op=del&id=<%=isfd.getId()%>&dirCode=<%=StrUtil.UrlEncode(dirCode)%>'">删除</a>
	  </td>
    </tr>
  </form>
    <%}%>
...
<table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    out.print(paginator.getCurPageBlock("media_list.jsp?"+querystr));
%></td>
  </tr>
</table>
<HR noShade SIZE=1>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><table width="100%" border=0 cellspacing=0 cellpadding=0 id="uploadTable">
      <form name=form1 action="media_list.jsp?action=<%=action%>&op=upload&dirCode=<%=StrUtil.UrlEncode(dirCode)%>" method="post" enctype="MULTIPART/FORM-DATA">
        <tr>
        ...
        <tr>
		  ...
		  <input name="name" size="10">
            <input name="submit2" type=submit value="添子目录">
			  ...
			  <a href="media_dir_modify.jsp?code=<%=StrUtil.UrlEncode(dirCode)%>">修改目录</a>&nbsp;
			  <%if (!dirCode.equals("root")) {%>
              <a href="javascript:if (confirm('您确定要删除吗？')) window.location.href='media_list.jsp?op=delDir&dirCode=<%=StrUtil.UrlEncode(dirCode)%>'">删除目录</a>
              <%}%></td>
        </tr>
      </form>
    </table></td>
...
String isRelatePath = cfg.getProperty("cms.isRelatePath"); 	
	%>
      <object classid="CLSID:DE757F80-F499-48D5-BF39-90BC8BA54D8C" codebase="<%=request.getContextPath()%>/activex/webedit.cab#version=6,0,2,1" width=400 height=170 align="middle" id="webedit">
        ...
        <param name="PostScript" value="<%=Global.virtualPath%>/forum/admin/media_list.jsp?op=uploadWebedit">
...
function doSort(orderBy) {
	if (orderBy==curOrderBy)
		if (sort=="asc")
			...
			sort = "asc";
	window.location.href = "media_list.jsp?dirCode=<%=StrUtil.UrlEncode(dirCode)%>&action=<%=action%>&op=<%=op%>&orderBy=" + orderBy + "&sort=" + sort;
}	
</script>
</html>