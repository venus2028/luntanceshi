...
if (checkStatus.equals(""))
	checkStatus = "" + MsgDb.CHECK_STATUS_DUSTBIN;
...
if (op.equals("del")) {
	...
	out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "dustbin.jsp?pageSize=" + pageSize + "&checkStatus=" + checkStatus));
}
...
if (op.equals("resume")) {
	...
	if (idsary!=null) {
		int len = idsary.length;
		for (int i=0; i<len; i++) {
			mm.resumeTopic(request, Long.parseLong(idsary[i]));
		}
	}
	out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "dustbin.jsp?pageSize=" + pageSize + "&checkStatus=" + checkStatus));
}
...
%>
  ...
  <br>
  <TABLE width="98%" align="center" cellPadding=0 cellSpacing=1 class="frame_gray">
    <FORM name=form1 action="dustbin.jsp" method=post onSubmit="return form1_onsubmit()">
      <TBODY>
        <TR>
          <TD height=22 
    colSpan=2 class="thead"><P align=center><SPAN class="text_title">
            <lt:Label res="res.label.forum.search" key="input_keywards"/>
          </SPAN> </P></TD>
        </TR>
        <TR bgColor=#f5f5f5>
          <TD height=24 align="right" bgcolor="#f5f5f5" 
    ...
    vAlign=top bgcolor="#f5f5f5">&nbsp;
        ...
        <TR bgColor=#f5f5f5>
          <TD 
     height=22 align="left" bgcolor="#f5f5f5"><P align=right><SPAN style="FONT-SIZE: 9pt">
    ...
    width=210 height=24 align="left" bgcolor="#f5f5f5"><P align=right><FONT style="FONT-SIZE: 9pt">
    ...
    width=210 height=23 align="left" bgcolor="#f5f5f5"><P align=right><FONT style="FONT-SIZE: 9pt" 
      color=#000000>
    ...
    vAlign=top bgcolor="#f5f5f5">&nbsp;
		  ...
		  <input name="checkStatus2" type="hidden" value="<%=checkStatus%>"></TD>
        </TR>
        <TR bgColor=#f5f5f5>
          <TD height=23 align="right" bgcolor="#f5f5f5" 
    ...
    align=right bgcolor="#f5f5f5"><FONT 
...
%>
  <TABLE width="98%" 
border=1 align=center cellPadding=1 cellSpacing=0 borderColor=#edeced class="thead">
  <TBODY>
  <TR height=25 class="td_title">
    <TD height="26" colSpan=3 align=middle noWrap class="text_title"><lt:Label res="res.label.forum.listtopic" key="topis_list"/></TD>
    <TD width=91 height="26" align=middle noWrap class="text_title"><lt:Label res="res.label.forum.listtopic" key="author"/></TD>
    <TD width=55 height="26" align=middle noWrap class="text_title"><lt:Label res="res.label.forum.listtopic" key="reply"/></TD>
    <TD width=55 height="26" align=middle noWrap class="text_title"><lt:Label res="res.label.forum.listtopic" key="hit"/></TD>
    <TD width=80 height="26" align=middle noWrap class="text_title"><lt:Label res="res.label.forum.listtopic" key="reply_date"/></TD>
        <TD width=91 height="26" align=middle noWrap class="text_title"><lt:Label res="res.label.forum.mytopic" key="board"/></TD>
  </TR>
</TBODY></TABLE>
<%
    ...
    <tr>
      ...
      <%if (recount>20){ %>
		...
		%>
          ...
          <% } %>
          <a target="_blank" title="<%=StrUtil.toHtml(md.getContent())%>" href="showtopic_tree.jsp?boardcode=<%=myboardcode%>&hit=<%=(hit+1)%>&rootid=<%=id%>"><%=StrUtil.toHtml(topic)%></a>
          <%
		// 计算共有多少页回贴
...
<table width="98%" border="0" cellspacing="1" cellpadding="3" align="center" class="9black">
    <tr> 
	  ...
	  <input name="button" type="button" onClick="doResume()" value="<lt:Label res="res.label.forum.topic_m" key="resume"/>">	  </td>
...
function doDel() {
	...
	window.location.href = "dustbin.jsp?op=del&pageSize=<%=pageSize%>&checkStatus=<%=checkStatus%>&ids=" + ids;
}
...
function doResume() {
	...
	window.location.href = "dustbin.jsp?op=resume&pageSize=<%=pageSize%>&checkStatus=<%=checkStatus%>&ids=" + ids;
}
