...
style="PADDING-RIGHT: 0px; BORDER-TOP: 1px; PADDING-LEFT: 0px; PADDING-BOTTOM: 0px; BORDER-LEFT: 1px; PADDING-TOP: 0px; BORDER-BOTTOM: 1px; BORDER-COLLAPSE: collapse; BORDER-RIGHT-WIDTH: 1px">
    ...
    >&nbsp;
        ...
        <TR bgColor=#f5f5f5>
          <TD height=23 align="right"><FONT style="FONT-SIZE: 9pt" 
      color=#000000>
...
&nbsp;&nbsp;&nbsp; </FONT></TD>
        ...
        <TR bgColor=#f5f5f5>
          <TD align=right width=210 bgColor=#f5f5f5 height=26><FONT 
...
%>
  ...
  <%
		...
		if (searchtype.equals("byauthor"))
		{
			...
			String names = ud.getNicksLike(searchwhat);
		
			if ( selauthor.equals("topicname"))
				sql = "select id from sq_message m, plugin_reply r where r.msg_id=m.id and m.name in (" + names + ")";
			else if ( selauthor.equals("replyname"))
				sql = "select id from sq_message m, plugin_reply r where r.msg_id=m.id and m.id in (select rootid from sq_message where name in (" + names + "))";
			else
				sql = "select id from sq_message m, plugin_reply r where r.msg_id=m.id and m.name in (" + names + ")";
		}
		else if (searchtype.equals("bykey")) {
			sql = "select id from sq_message m, plugin_reply r where r.msg_id=m.id and m.title like " + StrUtil.sqlstr("%"+searchwhat+"%");
		}
		else
			sql = "select id from sq_message m, plugin_reply r where r.msg_id=m.id";
		String sb="";
		if (selboard.equals("allboard")) {
			sb += " and check_status=" + checkStatus;
		}
		else
			sb = " and m.boardcode=" + StrUtil.sqlstr(selboard) + " and check_status=" + checkStatus;;
		sql += sb;
		String t1 = "";
...
border=1>
  ...
  <TR height=25 class="td_title">
    <TD height="26" colSpan=5 align=middle noWrap class="thead">主题</TD>
...
while (ri.hasNext()) {
    ...
    <tbody> 
    <tr>
      <td width=23 height="22" align=middle noWrap bgcolor=#f8f8f8><input name="ids" value="<%=id%>" type="checkbox"></td>
      <td noWrap align=left width=36 bgcolor=#f8f8f8><%=md.getId()%></td> 
      ...
      <td align=middle bgcolor=#ffffff><a href="javascript:openWin('distribute.jsp?id=<%=id%>', 480, 105)">分配</a></td>
    </tr>
    <tr> 
      <td 
