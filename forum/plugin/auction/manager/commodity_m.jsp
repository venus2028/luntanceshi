...
if (op.equals("del")) {
	long delId = ParamUtil.getLong(request, "msgRootId");
	MsgMgr mm = new MsgMgr();
	boolean re = mm.delTopicAbsolutely(application, request, delId);
	if (re) {
		out.print(StrUtil.Alert_Redirect("删除成功！", "commodity_m.jsp?userName=" + StrUtil.UrlEncode(userName)));
		return;
	}
	else {
...
%>  
  <tr> 
    <td valign="top"><br>
      ...
      <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" class="tableframe_gray">
      <tr align="center">
        <td width="4%" bgcolor="#F7F7F7">推荐</td>
        <td width="19%" height="22" bgcolor="#F7F7F7">商品名称</td>
      <td width="13%" height="22" bgcolor="#F7F7F7">所属目录</td>
        <td width="6%" bgcolor="#F7F7F7">数量</td>
        <td width="13%" bgcolor="#F7F7F7">状态</td>
        <td width="14%" height="22" bgcolor="#F7F7F7">销售类型</td>
      <td width="8%" bgcolor="#F7F7F7">销售日期</td>
        <td width="9%" bgcolor="#F7F7F7">截止日期</td>
        <td width="14%" bgcolor="#F7F7F7">操作</td>
      </tr>
<%
...
%>
	<form id="form<%=i%>" name="form<%=i%>" action="?op=modify" method="post">
      <tr align="center">
        ...
        <td height="22" bgcolor="#FFFFFF"><%=ad.getName()%><input type=hidden name=msgRootId value="<%=ad.getMsgRootId()%>">
        ...
        <td bgcolor="#FFFFFF"><input name=count value="<%=ad.getCount()%>" size=1></td>
        <td bgcolor="#FFFFFF">
		<%if (ad.getSellType()==ad.SELL_TYPE_SELL) {%>
			<select name="state">
	  ...
	  <%=DateUtil.format(ad.getBeginDate(), "yy-MM-dd HH:mm:ss")%>	  </td>
        ...
        <td height="28" align="center" bgcolor="#FFFFFF"><input type="submit" name="Submit" value="修改">
&nbsp;&nbsp;<a href="../../../showtopic.jsp?rootid=<%=ad.getMsgRootId()%>" target="_blank">查看</a>&nbsp;&nbsp;<a href="javascript:if (confirm('您确定要删除吗？请注意删除时会连同贴子一起删除！')) window.location.href='commodity_m.jsp?op=del&userName=<%=StrUtil.UrlEncode(userName)%>&msgRootId=<%=ad.getMsgRootId()%>'">删除</a></td>
      </tr></form>
<%}%>	  
    </table>
