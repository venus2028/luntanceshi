...
function shrink() {
   for(var i=0; i<document.images.length; i++) {
		...
		try {
			if (imgObj.tableRelate!=null && imgObj.tableRelate!="") {
...
<%
com.redmoon.forum.plugin.info.Directory dir = new com.redmoon.forum.plugin.info.Directory();
com.redmoon.forum.plugin.info.Leaf leaf = dir.getLeaf(com.redmoon.forum.plugin.info.Leaf.CODE_ROOT);
com.redmoon.forum.plugin.info.DirectoryView tv = new com.redmoon.forum.plugin.info.DirectoryView(request, leaf);
String pageUrl = ParamUtil.get(request, "pageUrl");
if (pageUrl.equals(""))
	tv.ListSimple(out, "", request.getContextPath() + "/forum/plugin/info/list.jsp?pluginCode=" + InfoUnit.code + "&typeCode=", "", "" ); // "tbg1", "tbg1sel");
else
	tv.ListSimple(out, "", request.getContextPath() + "/" + pageUrl, "", "" ); // "tbg1", "tbg1sel");
%>
</div>
<script>
shrink();
...
document.write('<tr><td valign="top" style="padding:3px" bgcolor="'+menuBGColor+'">' + document.getElementById("dirDiv").innerHTML + '</td><td align=center valign=top width=30><div width=30 title="分类信息" style="cursor:hand;filter:Alpha(opacity=80);color:#eeeeee;height:84px;background-image:url(<%=request.getContextPath()%>/forum/plugin/info/images/menu_bg.gif)"><b>' + barText + '</b></div></td></tr>');
