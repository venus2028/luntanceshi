...
<%
String sql = "select name,link,color,target,nav_type from cws_cms_nav where nav_type=" + Navigation.TYPE_FORUM + " order by orders";
...
<table style="BORDER-RIGHT: #a6a398 1px solid; BORDER-TOP: #a6a398 1px solid; BORDER-LEFT: #a6a398 1px solid; BORDER-BOTTOM: #a6a398 1px solid" cellSpacing="0" cellPadding="3" width="95%" align="center">
  <tbody>
    <tr>
      ...
      <td class="thead" noWrap width="19%"><img src="../../../admin/images/tl.gif" align="absMiddle" width="10" height="15">
      <lt:Label res="res.label.cms.nav_m" key="link"/></td>
      <td class="thead" noWrap width="14%"><img src="../../../admin/images/tl.gif" align="absMiddle" width="10" height="15">
      <lt:Label res="res.label.cms.nav_m" key="color"/></td>
      <td class="thead" noWrap width="14%"><img src="../../../admin/images/tl.gif" align="absMiddle" width="10" height="15">
      <lt:Label res="res.label.cms.nav_m" key="target"/></td>
      <td width="29%" noWrap class="thead"><img src="../../../admin/images/tl.gif" align="absMiddle" width="10" height="15">
...
while (ri.hasNext()) {
	...
	<form name="form<%=i%>" action="?op=edit" method="post">
      <td style="PADDING-LEFT: 10px">&nbsp;<img src="../../../admin/images/arrow.gif" align="absmiddle">&nbsp;<input name=newname value="<%=name%>"></td>
      ...
      <td>
	  [ <a href="javascript:form<%=i%>.submit()"><%=SkinUtil.LoadString(request,"op_edit")%></a> ] [ <a onClick="if (!confirm('<lt:Label res="res.label.cms.nav_m" key="confirm_del"/>')) return false" href="nav_m.jsp?op=del&name=<%=StrUtil.UrlEncode(name)%>&type=<%=Navigation.TYPE_FORUM%>"><%=SkinUtil.LoadString(request,"op_del")%></a> ] [<a href="nav_m.jsp?op=move&direction=up&name=<%=StrUtil.UrlEncode(name)%>&type=<%=Navigation.TYPE_FORUM%>"><lt:Label res="res.label.cms.nav_m" key="move_up"/></a>] [<a href="nav_m.jsp?op=move&direction=down&name=<%=StrUtil.UrlEncode(name)%>&type=<%=Navigation.TYPE_FORUM%>"><lt:Label res="res.label.cms.nav_m" key="move_down"/></a>] </td>
	  </form>
    </tr>
<%}%>
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
	<form name="addform1" action="?op=add" method="post">
      ...
      <td><input name=link value="" size="30">
        <input name="type" value="<%=Navigation.TYPE_FORUM%>" type="hidden"></td>
      <td><span class="stable">
