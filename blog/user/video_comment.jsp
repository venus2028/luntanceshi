...
if (op.equals("del")) {
	...
	VideoCommentDb pcd = new VideoCommentDb();
	pcd = (VideoCommentDb)pcd.getQObjectDb(new Long(delId));
	if (pcd.del()) {
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "video_comment.jsp?blogId=" + blogId));
	}
	else {
...
VideoDb md2 = new VideoDb();
while (ir.hasNext()) {
	mcd = (VideoCommentDb) ir.next(); 
...
&nbsp;&nbsp;[<a href="#" onClick="if (confirm('<lt:Label key="confirm_del"/>')) window.location.href='video_comment.jsp?op=del&delId=<%=mcd.getLong("id")%>&blogId=<%=blogId%>'"><lt:Label key="op_del"/>
</a>]&nbsp;&nbsp;&nbsp;歌曲：
<%
long videoId = mcd.getLong("video_id");
VideoDb md = (VideoDb)md2.getQObjectDb(videoId);
%>
<a href="../showvideo.jsp?videoId=<%=videoId%>" target="_blank"><%=StrUtil.toHtml(md.getString("title"))%></a>
</td>
      </tr>
      <tr> 
    ...
    <tr> 
      <td height="23" align="right"> 
          <%
				...
				out.print(paginator.getPageBlock(request,"video_comment.jsp?"+querystr));
				%>
      &nbsp;&nbsp;</td>
    </tr>
</table> 
