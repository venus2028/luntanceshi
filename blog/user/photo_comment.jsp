...
if (op.equals("del")) {
	...
	pcd = (PhotoCommentDb)pcd.getQObjectDb(new Long(delId));
	if (pcd.del()) {
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "photo_comment.jsp?blogId=" + blogId));
	}
	else {
	...
	return;
		...
		PhotoCommentDb mcd = new PhotoCommentDb();
		...
		if (photoId!=-1) {
			sql = "select id from " + mcd.getTable().getName() + " where blog_id=" + blogId + " and photo_id=" + photoId + " order by add_date desc";
		}
...
PhotoDb pd2 = new PhotoDb();
while (ir.hasNext()) {
	mcd = (PhotoCommentDb) ir.next(); 
...
&nbsp;&nbsp;[<a href="#" onClick="if (confirm('<lt:Label key="confirm_del"/>')) window.location.href='photo_comment.jsp?op=del&delId=<%=mcd.getLong("id")%>&blogId=<%=blogId%>'"><lt:Label key="op_del"/>
</a>]&nbsp;&nbsp;&nbsp;图片：
<%
long pId = mcd.getLong("photo_id");
PhotoDb pd = (PhotoDb)pd2.getPhotoDb(pId);
%>
<a href="../showphoto.jsp?photoId=<%=pd.getId()%>&blogId=<%=pd.getBlogId()%>" target="_blank"><%=StrUtil.toHtml(pd.getTitle())%></a>
</td>
      </tr>
      <tr> 
  ...
  <table width="98%" border="0" cellspacing="1" cellpadding="3" align="center" class="9black">
    <tr> 
      <td height="23" align="right"> 
          <%
				String querystr = "blogId=" + blogId + "&photoId=" + photoId;
				out.print(paginator.getPageBlock(request,"photo_comment.jsp?"+querystr));
				%>
      &nbsp;&nbsp;</td>
    </tr>
</table> 
