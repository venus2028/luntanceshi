...
<%@ page import="com.redmoon.blog.music.*"%>
...
<table width='100%' cellpadding='0' cellspacing='0' >
  <tr>
    <td class="head"><lt:Label res="res.label.blog.user.frame" key="music"/></td>
  </tr>
</table>
<%
DirDb dd = new DirDb();
...
if (op.equals("add")) {
	try {
		if (lm.create(application, request)) {
			out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "music.jsp?blogId=" + blogId));
		}
	}
...
while (ir.hasNext()) {
    ...
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      ...
      <td style="PADDING-LEFT: 10px"><input name=title style="width:90px;" value="<%=StrUtil.toHtml(mud.getString("title"))%>"></td>
      <td>
      ...
      <td title="<%=mud.getMusicUrl(request)%>">
	  <%if (StrUtil.getNullStr(mud.getString("music")).equals("")) {%>
	  <input name="link" value="<%=mud.getString("link")%>" size="30">
	  <%}else{%>
	  <input value="<%=mud.getMusicUrl(request)%>" size="30" readonly>
	  <%}%>	  </td>
	  ...
	  <script>
	  form<%=i%>.is_bk_music.value = "<%=mud.getInt("is_bk_music")%>";
	  </script>
	  </td>
      <td>
	  [ <a href="javascript:form<%=i%>.submit()"><lt:Label res="res.label.blog.user.photo" key="modify"/></a> ] [ <a onClick="if (!confirm('<lt:Label res="res.label.blog.user.photo" key="del_confirm"/>')) return false" href="music.jsp?op=del&id=<%=mud.getLong("id")%>&blogId=<%=blogId%>"><lt:Label res="res.label.blog.user.photo" key="del"/></a> ]
	  ...
	  <td><span style="PADDING-LEFT: 10px"><%=com.redmoon.forum.ForumSkin.formatDate(request, mud.getDate("add_date"))%>&nbsp;&nbsp;
		<a href="../showmusic.jsp?musicId=<%=mud.getLong("id")%>" target="_blank">
		<img src="../images/earphone.gif" border="0"></a>
      </span></td>
	  </form>
    </tr>
<%}%>
...
<table width="58%" border="0" align="center" cellpadding="3" cellspacing="0" style="BORDER-RIGHT: #a6a398 1px solid; BORDER-TOP: #a6a398 1px solid; BORDER-LEFT: #a6a398 1px solid; BORDER-BOTTOM: #a6a398 1px solid">
<form action="?op=add&blogId=<%=blogId%>" method="post" enctype="multipart/form-data" name="addform1">
  <tr>
    <td height="24" colspan="2" align="center" class="thead">上传歌曲</td>
    </tr>
  <tr>
    <td height="24">分类</td>
    <td>
  ...
  <tr>
    <td height="24">
      <lt:Label res="res.label.blog.user.photo" key="music"/>    </td>
  ...
  <tr>
    <td height="24">序号</td>
    <td><input name="sort" size="2" value="0"></td>
  </tr>
  <tr>
    <td height="24">背景音乐</td>
    <td><select name="is_bk_music">
      <option value="1">
      <lt:Label key="yes"/>
      </option>
