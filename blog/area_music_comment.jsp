...
pd = (MusicDb)pd.getQObjectDb(new Long(musicId));
long blog_id = pd.getLong("blog_id");
...
<%} 
if (pd.getInt("is_locked")==1) {
} else{%>
<br />
<a name='form'></a>
<form name="frmAnnounce" action="showmusic.jsp" method=post onsubmit="return formCheck()">
<table style="BORDER-COLLAPSE: collapse; margin-top:10px" bordercolor=#d3d3d3 height=120 cellspacing=0 cellpadding=5 width="87%" align=center border=1>
    <tbody>
      ...
      <tr bgcolor=#ffffff>
        <td width="744" align="center" valign=top><div align=left>
        <%
		...
		%>
            ...
            <br>
            <input name="musicId" value="<%=musicId%>" type="hidden" />
            <input name="music_id" value="<%=musicId%>" type="hidden" />
            <input name="blog_id" value="<%=blog_id%>" type="hidden" />
...
%>
            ...
            <%}%>
        </div><input tabindex=4 type=submit value="<lt:Label res="res.label.forum.showblog" key="comment"/>" name=submit1>
</td>
      </tr>
  </tbody>
</table>
