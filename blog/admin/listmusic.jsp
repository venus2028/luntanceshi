...
if (op.equals("del")) {
	...
	MusicDb ld = new MusicDb();
    ld = (MusicDb)ld.getQObjectDb(new Long(id));
    boolean re = ld.del();
	if (re)
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_success"), "listmusic.jsp"));
	else
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_fail"), "listmusic.jsp"));
	return;
}
...
<TABLE cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
  <TR>
    <TD class=head>管理歌曲</TD>
  </TR></TBODY></TABLE>
<br>
<%
int pagesize = 20;

MusicDb pd = new MusicDb();
...
<table width="98%" height="227" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
  <tr>
	...
	<table width="44%" border="0" align="center" cellpadding="0" cellspacing="0" class="p9">
      <form name="form1" action="listmusic.jsp?action=search" method="post">
        <tr>
          <td align="center">
...
%>
		...
		<a class="tooltips" href="../showmusic.jsp?blogId=<%=pd.getLong("blog_id")%>&id=<%=pd.getLong("id")%>" target=_blank>
		<%=pd.getString("title")%>
		</a>			  </td>
			  ...
			  &nbsp;&nbsp;<a target=_blank href="<%=pd.getMusicUrl(request)%>">打开</a><a href="listmusic.jsp?op=del&id=<%=pd.getLong("id")%>&CPages=<%=curpage%>">
...
<%
    ...
    out.print(paginator.getPageBlock(request, "listmusic.jsp?"+querystr));
%>
            </div></td>
          </tr>
    </table>
        ...
        <form name=formSubDir target="leftFileFrame" action="music_left.jsp?op=AddChild" method="post">
          <tr>
            <td align="right" valign=top class=tablebody1><input name="name" size="10">
                ...
                <a href="javascript:if (confirm('您确定要删除吗？')) window.location.href='music_list.jsp?op=delDir&dirCode=<%=StrUtil.UrlEncode(dir_code)%>'">删除目录</a>
                <%}%>
			</td>
          </tr>
        </form>
