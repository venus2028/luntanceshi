...
if (op.equals("del")) {
	...
	try {
		if (docmanager.del(request, id, privilege, true)){
		    String redirectUrl = "software_list.jsp?pageSize=" + pageSize + "&op=search";
			if(!titleKey.equals(""))
...
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
 <form name="form1" method="post" action="?op=search">
  <tr> 
    <td height=20 align="left" class="thead">搜索符合条件软件</td>
  </tr>
  <tr> 
    <td valign="top"><br>
...
sql = "select d.id from document d, cws_cms_software_doc s where s.doc_id=d.id";
...
<table style="BORDER-RIGHT: #a6a398 1px solid; BORDER-TOP: #a6a398 1px solid; BORDER-LEFT: #a6a398 1px solid; BORDER-BOTTOM: #a6a398 1px solid" cellspacing="1" cellpadding="3" width="98%" align="center">
  <tbody>
    <tr>
      ...
      <td width="8%" align="center" nowrap class="thead">下载</td>
...
%>
      ...
      <td align="center"><%=sdd.getDownloadCount()%>
      </td>
      <td align="center"><%
	  ...
	  %></td>
      <td align="left">
	  <a target="_blank" href="<%=request.getContextPath()%>/<%=DocumentMgr.getWebEditPage()%>?op=edit&id=<%=id%>&dir_code=<%=StrUtil.UrlEncode(class1)%>">[<lt:Label res="res.label.cms.doc" key="edit"/>]</a> 
	  <a onClick="return confirm('您确定要删除吗？')" href="software_list.jsp?pageSize=<%=pageSize%>&op=del&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[<lt:Label res="res.label.cms.doc" key="del"/>]</a>&nbsp;
  	  <%if (doc.getExamine()!=Document.EXAMINE_PASS) {%>
	  <a href="software_list.jsp?pageSize=<%=pageSize%>&op=passExamine&examine=<%=Document.EXAMINE_PASS%>&ids=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[通过]</a>
	  <%}else{%>
	  <a href="software_list.jsp?pageSize=<%=pageSize%>&op=passExamine&examine=<%=Document.EXAMINE_NOT%>&ids=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[未审核]</a>
	  <%}%>  
	  <%if (docLevel != Document.LEVEL_TOP) {%>
      <a onClick="return confirm('您确定要置顶吗？')" href="software_list.jsp?pageSize=<%=pageSize%>&op=setOnTop&level=<%=Document.LEVEL_TOP%>&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[置顶]</a>
      <%}else{%>
      <a onClick="return confirm('您确定要取消置顶吗？')" href="software_list.jsp?pageSize=<%=pageSize%>&op=setOnTop&level=0&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[取消置顶]</a>
      <%}%></td>
    </tr>
    <%}%>
  </tbody>
...
<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td width="49%" align="left"><input name="button3" type="button" onClick="selAllCheckBox('ids')" value="<lt:Label res="res.label.forum.topic_m" key="sel_all"/>">
      &nbsp;&nbsp;
      ...
      &nbsp;
      <input name="button32" type="button" onClick="sel()" value="<lt:Label key="ok"/>"></td>
...
<%
	...
	if(!querystr.equals(""))
		...
		querystr = "op=" + op;
    out.print(paginator.getCurPageBlock("software_list.jsp?pageSize="+ pageSize + "&" + querystr));
%>
</td>
  </tr>
</table>
...
function doOperation() {
	...
	window.location.href = "software_list.jsp?pageSize=" + form1.pageSize.value + "&op=moveBoard&tDirCode=" + tDirValue + "&ids=" + ids + "&<%=strDoOperation%>";
...
function doDel(querystr) {
	...
	if(querystr != "")
		window.location.href = "software_list.jsp?pageSize=" + form1.pageSize.value + "&op=delBatch&ids=" + ids + "&" + querystr;
	else
		window.location.href = "software_list.jsp?pageSize=" + form1.pageSize.value + "&op=delBatch&ids=" + ids;
}
