	...
	* v6  :发现并修复一个BUG（之前当有一个队列比较结束时，没有将前一个相等的位置到当前位置的所有节点进行CompareText）.
	* v7  :发现并修复一个BUG（现象：节点内容重复。分割点错误，当长度等于0时，退出循环，见line133,154.）
	* v8  :发现并修复一个BUG（现象：抽出顿号。屏蔽相同节点细比出错.见line 38~42.）
	*/
