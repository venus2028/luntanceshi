...
int type = 0;
VideoStoreDirMgr dir = new VideoStoreDirMgr();
VideoStoreDirDb leaf = dir.getVideoStoreDirDb(code);
name = leaf.getName();
...
String parent_name = "";
VideoStoreDirDb pisdd = dir.getVideoStoreDirDb(parent_code);
if (pisdd!=null)
      ...
      <TD align="center" style="PADDING-LEFT: 10px"><table class="frame_gray" width="48%" border="0" cellpadding="0" cellspacing="1">
        <tr>
          <td align="center"><table width="100%">
            <form name="form1" method="post" action="video_left.jsp?op=modify" target="leftFileFrame" onClick="return form1_onsubmit()">
              <tr>
...
<select name="parentCode">
  <%
									VideoStoreDirDb rootlf = leaf.getVideoStoreDirDb(VideoStoreDirDb.ROOTCODE);
									VideoStoreDirView dv = new VideoStoreDirView(rootlf);
...
function del() {
	if (confirm("您确定要删除吗？目录下的所有文件也将会一起被删除！")) {
		window.parent.leftFileFrame.location.href = "video_left.jsp?op=del&code=" + "<%=StrUtil.UrlEncode(code)%>";
	}
}

function move(fx) {
	window.parent.leftFileFrame.location.href = "video_left.jsp?op=move&direction=" + fx + "&code=" + "<%=StrUtil.UrlEncode(code)%>";
}
</script>
</html>
