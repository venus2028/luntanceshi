...
<table width="98%" border='0' align="center" cellpadding='3' cellspacing='0' class="frame_gray">
  ...
  <tr>
    <td width="14%" height=26 align="left">请选择一级目录：</td>
    <td width="86%" height="26" align="left">
<select name="site_code" onChange="siteChange()">
	<%
		Iterator ir = null;
		String tempCode = ParamUtil.get(request, "site");
		Leaf lf = new Leaf();
		lf = lf.getLeaf("root");
		Vector v = lf.getChildren();
		ir = v.iterator();
		while(ir.hasNext()){
			lf = (Leaf)ir.next(); 
			String parentCode = lf.getCode();
			if(tempCode.equals(parentCode)){%>
				<option value="<%=parentCode%>" selected="selected"><%=lf.getName()%></option>
			<%}
			%>
			<option value="<%=parentCode%>"><%=lf.getName()%></option>
		<%}	
	%>
	</select>	
	</td>
  ...
  <tr>
    <td height=26 align="left">请选择相应目录：</td>
  ...
  <tr>
    <td height=26 colspan="2" align="center">
      <input name="submit" type="submit" value="添加文章">    </td>
  </tr>
</table>
</form>
...
<script>
function siteChange(){
	var site = document.addform.site_code.value;
	window.location.href = "doc_add.jsp?site="+site;
}
</script>
</html>
