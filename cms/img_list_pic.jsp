...
if (op.equals("upload")) {
	...
	try {
		re = isfm.create(application, request);
		if (re)
			out.print(StrUtil.Alert_Redirect("操作成功！", "img_list_pic.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));	
		else
			out.print(StrUtil.Alert_Redirect("操作失败！", "img_list_pic.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));		
	}
	catch (ErrMsgException e) {
		out.print(StrUtil.Alert_Redirect(e.getMessage(), "img_list_pic.jsp?dirCode=" + StrUtil.UrlEncode(dirCode)));	
		return;
	}
}
	  ...
	  if (!op.equals("search")) {
	  	if (leaf!=null && leaf.isLoaded()) {
			...
			while (!parentcode.equals("root")) {
				...
				navstr = "<a href='img_list_pic.jsp?action=" + action + "&dirCode=" + StrUtil.UrlEncode(plf.getCode()) + "'>" + plf.getName() + "</a>&nbsp;>>&nbsp;" + navstr;
		...
		%>
      &nbsp;&nbsp;<a href="img_list.jsp?dirCode=<%=StrUtil.UrlEncode(dirCode)%>&action=<%=action%>">列表</a></td>
    </tr>
  </tbody>
</table>
<br>
<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0" class="p9">
  <form name="form1" action="img_list_pic.jsp?op=search" method="post">
    <tr>
...
%>
    <tr>
      <td align="left" nowrap class="tbg1" style="PADDING-LEFT: 5px; height:22px"><img src="images/folder_01.gif" width="16" height="12">&nbsp;<a href="img_list_pic.jsp?dirCode=<%=StrUtil.UrlEncode(leaf.getParentCode())%>">..</a></td>
    </tr>	
<%
}
...
%>	
    <tr>
      <td align="left" nowrap class="tbg1" style="PADDING-LEFT: 5px; height:22px"><img src="images/folder_01.gif" width="16" height="12">&nbsp;<a href="img_list_pic.jsp?dirCode=<%=StrUtil.UrlEncode(isdd.getCode())%>"><%=isdd.getName()%></a></td>
    </tr>
<%}%>	
    <tr>
...
while (ri.hasNext()) {
	...
	<%}%>
	  <form name="form<%=k%>" action="img_list_pic.jsp?op=rename&dir=<%=StrUtil.UrlEncode(dir)%>" method="post">
		  <td colspan="4" align="center" valign="bottom"><table width="100%" height="100" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td align="center"><img src="../<%=isfd.getVisualPath() + "/" + isfd.getDiskName()%>" width="100">
            ...
            <tr>
              <td height="25" align="center"><a href="#" onClick="form<%=k%>.submit()">重命名</a>&nbsp;<a href="#" onClick="if (confirm('您确定要删除吗？')) window.location.href='img_list_pic.jsp?op=del&id=<%=isfd.getId()%>&dirCode=<%=StrUtil.UrlEncode(dirCode)%>&dir=<%=StrUtil.UrlEncode(dir)%>'">删除</a>&nbsp;
...
<table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    out.print(paginator.getCurPageBlock("img_list_pic.jsp?"+querystr));
%></td>
  </tr>
</table>
<HR noShade SIZE=1>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td><table width="100%" border=0 cellspacing=0 cellpadding=0 id="uploadTable">
      <form name=form1 action="img_list_pic.jsp?op=upload&dirCode=<%=StrUtil.UrlEncode(dirCode)%>&dir=<%=StrUtil.UrlEncode(dir)%>" method="post" enctype="MULTIPART/FORM-DATA">
        <tr>
    ...
    <td><table width="100%" border=0 cellspacing=0 cellpadding=0 id="uploadTable">
      <form name=formSubDir target="leftFileFrame" action="img_left.jsp?op=AddChild&dir=<%=StrUtil.UrlEncode(dir)%>" method="post">
        <tr>
		  ...
		  <input name="name" size="10">
            <input name="submit2" type=submit value="添子目录">
			  ...
			  <a href="img_dir_modify.jsp?code=<%=StrUtil.UrlEncode(dirCode)%>">修改目录</a></td>
        </tr>
      </form>
    </table></td>
  </tr>
...
String isRelatePath = cfg.getProperty("cms.isRelatePath"); 	
	%>
      <object classid="CLSID:DE757F80-F499-48D5-BF39-90BC8BA54D8C" codebase="<%=request.getContextPath()%>/activex/webedit.cab#version=6,0,2,1" width=400 height=170 align="middle" id="webedit">
        ...
        <param name="PostScript" value="<%=Global.virtualPath%>/cms/img_list_pic.jsp?op=uploadWebedit">
