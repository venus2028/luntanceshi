...
<%
String sql = "select id from cms_comment";

if (!siteCode.equals(""))
	sql += " where site_code=" + StrUtil.sqlstr(siteCode);
	...
	if (fDirCode.equals("")) {
		if (siteCode.equals("")) {
			if (kind.equals("content"))
				...
				sql += " where nick like " + StrUtil.sqlstr("%" + what + "%");
		} else {
			if (kind.equals("content"))
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <form name="formSearch" method="get" action="comment_all_m.jsp">
    <tr>
	  ...
	  if (!siteCode.equals("") && fDirCode.equals("")) {
	  	Leaf siteLeaf = dir.getLeaf(siteCode);
		%>
		当前为：<%=siteLeaf.getName()%>&nbsp;&nbsp;<a href="comment_all_m.jsp">查看全部</a>&nbsp;&nbsp;
	  <%}
	  else {
	  %>
        ...
        <%
		Leaf lf = dir.getLeaf(Leaf.ROOTCODE);
...
<table style="BORDER-RIGHT: #a6a398 1px solid; BORDER-TOP: #a6a398 1px solid; BORDER-LEFT: #a6a398 1px solid; BORDER-BOTTOM: #a6a398 1px solid" cellSpacing="0" cellPadding="3" width="98%" align="center">
  <tbody>
    <tr>
      ...
      <td width="10%" noWrap class="thead" style="PADDING-LEFT: 10px;cursor:pointer" onClick="doSort('site_code')"><img src="images/tl.gif" align="absMiddle" width="10" height="15">站点
		<%if (orderBy.equals("site_code")) {
			if (sort.equals("asc")) 
...
while (ir.hasNext()) {
	...
	if (doc!=null) {
		...
		doc = new Document();
	Leaf lf = dir.getLeaf(cmt.getSiteCode());
%>
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      ...
      <td style="PADDING-LEFT: 10px"><a href="comment_all_m.jsp?siteCode=<%=StrUtil.UrlEncode(lf.getCode())%>&orderBy=<%=orderBy%>&sort=<%=sort%>"><%=lf.getName()%></a></td>
