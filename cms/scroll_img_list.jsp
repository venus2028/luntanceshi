...
function SelectImage(urlObject) {
	urlObj = urlObject;
	openWin("img_frame.jsp?action=selectImage", 800, 600);
}
...
if (op.equals("del")) {
    ...
    boolean re = ld.del();
	if (re)
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_success"), "scroll_img_list.jsp?siteCode=" + siteCode + "&kind=" + kind));
	else
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_fail"), "scroll_img_list.jsp?siteCode=" + siteCode + "&kind=" + kind));
	return;
}
	...
	try {
		if (qom.create(request, sad, "site_scroll_img_create")) {
			out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "scroll_img_list.jsp?siteCode=" + siteCode + "&kind=" + kind));
		}
		else {
...
<DIV id="tabBar">
  <div class="tabs">
    <ul>
      <li id="menu1"><a href="<%=request.getContextPath()%>/cms/scroll_img_list.jsp?kind=<%=SiteScrollImgDb.KIND_SCROLL%>">滚动</a></li>
      <li id="menu2"><a href="<%=request.getContextPath()%>/cms/scroll_img_list.jsp?kind=<%=SiteScrollImgDb.KIND_SWITCH%>">变换</a></li>
    </ul>
  </div>
</DIV>
<script>
...
%>
        <form id="frm<%=i%>" name="frm<%=i%>" action="?op=modify" method="post">
          <tr align="center">
            ...
            <td height="22" bgcolor="#FFFFFF"><a href="scroll_img_edit.jsp?siteCode=<%=siteCode%>&id=<%=pd.getLong("id")%>">编辑</a>&nbsp;&nbsp;
			<a href="#" onClick="if (confirm('您确定要删除吗？')) window.location.href='scroll_img_list.jsp?op=del&id=<%=pd.getLong("id")%>&siteCode=<%=StrUtil.UrlEncode(siteCode)%>&kind=<%=kind%>&CPages=<%=curpage%>'"><lt:Label res="res.label.blog.admin.blog" key="del"/>
            </a> </td>
          </tr>
        </form>
        <%}%>
...
%>
      ...
      <table width="73%" align="center" class="frame_gray">
        <form id=formAdd name=formAdd action="?op=add&siteCode=<%=siteCode%>" method=post>
          ...
          <tr>
            <td width="9%" height="22">文字：</td>
            <td width="31%"><input name="title"></td>
            <td width="11%">地址：</td>
          ...
          <tr>
            <td height="22" colspan="4" align="left"><p>帮助：
			    <%
			String var = "";
			if (kind.equals(SiteScrollImgDb.KIND_SCROLL)) {
				var = "scrollImg";
			...
			%>
                <br>
1、在模板中提取的方法是：<span class="flash_img">{$cms.<%=var%>(speed=30,title=y,siteCode=***)}<br>
2、用JS提取的方法是：<span class="ad_right_box">&lt;script src=&quot;js.jsp?var=<%=var%>&amp;siteCode=***&quot;&gt;&lt;/script&gt;<br>
3、序号是用来排序的，序号大的排在前面</span></span></p>
            <p>注：<span class="ad_right_box">siteCode为子站点编码，不填表示提取首页的滚动图片</span></p></td>
          </tr>
        </form>
      </table>
      <br>
