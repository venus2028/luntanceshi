...
%>
<jsp:useBean id="dm" scope="page" class="cn.js.fan.module.cms.ImgStoreDirMgr"/>
<%
...
if (op.equals("AddChild")) {
	...
	if (re) {
		out.print(StrUtil.Alert_Redirect("操作成功！", "img_left.jsp?dir=" + dir));
	}
	return;
}
if (op.equals("del")) {
...
ImgStoreDirDb leaf = dm.getImgStoreDirDb(root_code);
if (op.equals("repair")) {
	dm.repairTree(leaf);
	leaf = dm.getImgStoreDirDb(root_code);
}
...
if (!dir.equals("")) {
	...
	if (lf.getType()==Leaf.TYPE_SUB_SITE) {
		leaf = dm.getImgStoreDirDb(lf.getCode());
	}
	else {
...
ImgStoreDirView dv = new ImgStoreDirView(leaf);
dv.ListSimple(out, "mainFileFrame", "img_list.jsp?action=" + action + "&isFromLeftFrame=true&dir=" + StrUtil.UrlEncode(dir) + "&dirCode=", "", "" );
%></TD>
    </TR>
  </TBODY>
</TABLE>
