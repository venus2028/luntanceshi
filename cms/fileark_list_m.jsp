...
while (ri.hasNext()) {
    ...
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      ...
      <td><a href="../fckwebedit.jsp?op=edit&id=<%=rr.getInt("id")%>&dir_code=<%=StrUtil.UrlEncode((String)rr.get(1))%>&dir_name=<%=StrUtil.UrlEncode(dir_name)%>">[ 编辑 ]</a> <a onClick="return confirm('您确定要删除吗？')" href="document_list_m.jsp?op=del&id=<%=rr.getString(3)%>&dir_code=<%=dir_code%>&dir_name=<%=StrUtil.UrlEncode(dir_name)%>">[ 删除 ]</a> <a href="doc_template_show.jsp?id=<%=rr.getInt("id")%>">[ 查看 ]</a> <a href="javascript:selTemplate(<%=rr.getInt("id")%>)">[ 选用 ]</a> </td>
    </tr>
    <%}%>
  </tbody>
</table>
...
<DIV style="WIDTH: 95%" align=right>
  <INPUT 
onclick="javascript:location.href='fckwebedit.jsp?op=add&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&dir_name=<%=StrUtil.UrlEncode(dir_name, "utf-8")%>';" type=image 
