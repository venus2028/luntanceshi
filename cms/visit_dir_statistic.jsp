...
<%		
	...
	String sql = "select count(*) as s, dir_code from " + bvld.getTable().getName();
	JdbcTemplate jt = new JdbcTemplate();
  ...
  <tr> 
	...
	%>
      <tr align="center">
		...
		<a href="visit_dir_statistic_day_detail.jsp?dirCode=<%=lf.getCode()%>&beginDate=<%=StrUtil.UrlEncode(beginDate)%>&endDate=<%=StrUtil.UrlEncode(endDate)%>"><%=dirName%></a>
		</td>
...
function del() {
	if (confirm("您确定要删除吗？")) {
		window.location.href = "visit_dir_statistic.jsp?op=del&beginDate=" + fmFilter.beginDate.value + "&endDate=" + fmFilter.endDate.value;
	}
}
</script>
</html>                            
