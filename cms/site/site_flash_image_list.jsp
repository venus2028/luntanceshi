...
if (op.equals("del")) {
    ...
    boolean re = ld.del();
	if (re)
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_success"), "site_flash_image_list.jsp?siteCode=" + siteCode));
	else
		out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request,"res.common", "info_op_fail"), "site_flash_image_list.jsp?siteCode=" + siteCode));
	return;
}
...
if (op.equals("add")) {
	...
	try {
		if (qom.create(request, sad, "site_flash_image_create")) {
			out.print(StrUtil.Alert_Redirect(SkinUtil.LoadString(request, "info_op_success"), "site_flash_image_list.jsp?siteCode=" + siteCode));
		}
		else {
...
%>
        <form id="frm<%=i%>" name="frm<%=i%>" action="?op=modify" method="post">
          <tr align="center">
            ...
            <td height="22" bgcolor="#FFFFFF"><a href="site_flash_image_edit.jsp?siteCode=<%=siteCode%>&id=<%=pd.getLong("id")%>">编辑</a>&nbsp;&nbsp;
			<a href="#" onClick="if (confirm('您确定要删除吗？')) window.location.href='site_flash_image_list.jsp?op=del&id=<%=pd.getLong("id")%>&siteCode=<%=StrUtil.UrlEncode(siteCode)%>&CPages=<%=curpage%>'"><lt:Label res="res.label.blog.admin.blog" key="del"/>
            </a> </td>
          </tr>
        </form>
        <%}%>
