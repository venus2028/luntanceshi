...
<DIV id="tabBar">
  <div class="tabs">
    <ul>
      <li id="menu3"><a href="<%=request.getContextPath()%>/cms/site/visit_dir_statistic.jsp?siteCode=<%=siteCode%>">目录访问统计</a></li>
      <li id="menu4"><a href="<%=request.getContextPath()%>/cms/site/visit_doc_statistic.jsp?siteCode=<%=siteCode%>">文章访问统计</a></li>
    </ul>
  </div>
</DIV>
<script>
...
<%
	...
	VisitDirLogDb vdld = new VisitDirLogDb();
...
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
  <tr> 
    <td height=20 align="left" class="thead">目录 - <%=lf.getName()%> 访问统计</td>
  </tr>
  <tr> 
    <td valign="top"><br>
      <TABLE width="92%" border=0 align=center cellPadding=0 cellSpacing=1>
      <TBODY>
      <form name="fmFilter" action="visit_dir_statistic_day_detail.jsp?op=search&dirCode=<%=dirCode%>" method="post">
        <TR>
...
<script type="text/javascript">
	...
	String sql = "select count(*) from " + vdld.getTable().getName() + " where dir_code=? and add_date>=? and add_date<?";
	int dayCount = 0;
	JdbcTemplate jt = new JdbcTemplate();
	d = DateUtil.addDate(d, 1);
	while (DateUtil.datediffMinute(ed, d)>=-1) {
		ri = jt.executeQuery(sql, new Object[] {dirCode, oldd, d});
		if (ri.hasNext()) {
  ...
  <tr>
    <td align="center"><%
JFreeChart chart = ChartFactory.createBarChart3D("目录日访问统计图", 
                  "日期",
                  "访问量",
                  dataset,
