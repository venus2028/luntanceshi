...
if (op.equals("del")) {
	...
	try {
		if (docmanager.del(request, id, privilege, true)){
		    String redirectUrl = "document_batch_m.jsp?siteCode=" + StrUtil.UrlEncode(siteCode) + "&pageSize=" + pageSize + "&op=search";
...
<table width="98%" border='0' align="center" cellpadding='0' cellspacing='0' class="frame_gray">
 <form name="form1" method="get" action="document_batch_m.jsp">
  <tr> 
...
if (op.equals("search")) {
	...
	if(!condition.equals(""))
		condition += " and site_code=" + StrUtil.sqlstr(siteCode);
	else
		condition += " site_code=" + StrUtil.sqlstr(siteCode);
	if(!condition.equals("")){
...
%>
	  ...
	  <a onClick="return confirm('您确定要删除吗？')" href="document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=<%=pageSize%>&op=del&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[<lt:Label res="res.label.cms.doc" key="del"/>]</a>&nbsp;
  	  <%if (doc.getExamine()!=Document.EXAMINE_PASS) {%>
	  <a href="document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=<%=pageSize%>&op=passExamine&examine=<%=Document.EXAMINE_PASS%>&ids=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[通过]</a>
	  <%}else{%>
	  <a href="document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=<%=pageSize%>&op=passExamine&examine=<%=Document.EXAMINE_NOT%>&ids=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[未审核]</a>
	  <%}%>  
	  <%if (docLevel != Document.LEVEL_TOP) {%>
      <a onClick="return confirm('您确定要置顶吗？')" href="document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=<%=pageSize%>&op=setOnTop&level=<%=Document.LEVEL_TOP%>&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[置顶]</a>
      <%}else{%>
      <a onClick="return confirm('您确定要取消置顶吗？')" href="document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=<%=pageSize%>&op=setOnTop&level=0&id=<%=id%><%if(!querystr.equals("")) out.print("&" + querystr);%>">[取消置顶]</a>
      <%}%></td>
    </tr>
    <%}%>
  </tbody>
...
<%
	...
	if(!querystr.equals(""))
		...
		querystr = "op=" + op;
    out.print(paginator.getCurPageBlock("document_batch_m.jsp?siteCode=" + StrUtil.UrlEncode(siteCode) + "&pageSize="+ pageSize + "&" + querystr));
%>
</td>
  </tr>
</table>
...
function doOperation() {
	...
	window.location.href = "document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=" + form1.pageSize.value + "&op=moveBoard&tDirCode=" + tDirValue + "&ids=" + ids + "&<%=strDoOperation%>";
...
function doDel(querystr) {
	...
	if(querystr != "")
		window.location.href = "document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=" + form1.pageSize.value + "&op=delBatch&ids=" + ids + "&" + querystr;
	else
		window.location.href = "document_batch_m.jsp?siteCode=<%=StrUtil.UrlEncode(siteCode)%>&pageSize=" + form1.pageSize.value + "&op=delBatch&ids=" + ids;
}
