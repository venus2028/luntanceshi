...
%>
		<li><lt:Label res="res.label.forum.showtopic" key="topic_count"/><%=addcount%></li>
		...
		<li><lt:Label res="res.label.forum.showtopic" key="online_time"/><%=(int)user.getOnlineTime()%><lt:Label res="res.label.forum.showtopic" key="hour"/></li>
		<%}%>
...
if (com.redmoon.blog.Config.getInstance().isBlogOpen) {
	...
	while (qoi.hasNext()) {
      ...
      <tr>
		...
		<%
		cn.js.fan.module.cms.Config cmscfg = new cn.js.fan.module.cms.Config();
		if (cmscfg.getBooleanProperty("cms.is_subsite")) {%>
		<lt:Label res="res.label.usercenter" key="site_mgr"/>：
		<%
		SiteDb sd = new SiteDb();
		Vector vsite = sd.getSiteDbsManageredByUser(user.getName());
		Iterator ir = vsite.iterator();
		while (ir.hasNext()) {
			sd = (SiteDb)ir.next();
		%>
			<a href='cms/site/frame.jsp?op=siteManagerLogin&siteCode=<%=sd.getString("code")%>' target=_blank><%=sd.getString("name")%></a>&nbsp;
		<%}%>
		[<a href="site_apply.jsp"><lt:Label res="res.label.usercenter" key="site_create"/></a>]
		<%}%>
		</td>
        </tr>
      <tr>
...
MiniPluginUnit indexUnit = mpm.getMiniPluginUnit("advertiseLink");
if (indexUnit!=null && indexUnit.isPlugin()) {
%>
<lt:Label res="res.label.usercenter" key="ad_link"/>：&nbsp;&nbsp;&nbsp;<%=Global.getFullRootPath()%>/ad_user_link.jsp?userId=<%=StrUtil.UrlEncode(user.getId())%>
<%}%>		</td>
        </tr>
      <tr>
...
<%if (com.redmoon.blog.Config.getInstance().isBlogOpen || com.redmoon.forum.Config.getInstance().getBooleanProperty("forum.isWebeditTopicEnabled")) {%>
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td width="13%" align="left"><lt:Label res="res.label.usercenter" key="disk_space"/></td>
              <td width="41%" height="22"><table width="100%" class="tableBorder1">
                  <tr>
	...
	%>
              ...
              <td class="border0" width="46%" align="left">&nbsp;
                <lt:Label res="res.label.usercenter" key="disk_space_used"/><%=wpercent%>%&nbsp;(<%=user.getDiskSpaceUsed()/1024%>K)&nbsp;
                <lt:Label res="res.label.usercenter" key="disk_space_allowed"/><%=NumberUtil.round((double)spaceAllowed/1024000, 1)%>M</td>
              </tr>
              </table>
<%}%>				</td>
        </tr>
